...
const PlanAi = () => {
    ...
    const activeSuggestions = (editTrip?.suggestions || duplicateTrip?.suggestions || suggestions);
    ...
    const [tripTitle, setTripTitle] = useState(
        (destination || localDestination) ? (destination || localDestination) + " AI Trip" : "AI Generated Trip"
    );
    ...
    const generateAIContent = async () => {
        ...
        const safeExtractText = (result: any): string => {
            try {
                ...
                if (result?.response && typeof result.response === "object" && typeof result.response.text === "function") {
                ...
                // Candidate path
                const cand = result?.response?.candidates?.[0]?.content?.parts?.[0]?.text;
        ...
        const extractJson = (raw: string): any | null => {
            ...
            try {
                ...
                console.warn("[AI] primary JSON parse failed, attempting aggressive cleanup", e);
        ...
        try {
            ...
            const currentPlanPreview = sections.map((s) => ({
                id: s.id,
                title: s.title,
                startDate: s.startDate ? s.startDate.toISOString().substring(0, 10) : null,
...
CRITICAL: Return ONLY valid JSON. No markdown, no explanations.`;
            ...
            if (Array.isArray(aiData.sections) && aiData.sections.length > 0) {
                const newSections: Section[] = aiData.sections.map((section: any, index: number) => {
                    let sectionStartDate = tripStartDate;
                    let sectionEndDate = tripEndDate;

                    // Use provided dates if valid
                    const parsedStart = section.startDate ? new Date(section.startDate) : null;
                    const parsedEnd = section.endDate ? new Date(section.endDate) : null;
                    if (parsedStart && !isNaN(parsedStart.getTime())) sectionStartDate = parsedStart;
                    if (parsedEnd && !isNaN(parsedEnd.getTime())) sectionEndDate = parsedEnd;

                    // Distribute if multiple sections & trip window exists
                    if (tripStartDate && tripEndDate && aiData.sections.length > 1) {
                        const totalDays =
                            Math.ceil((tripEndDate.getTime() - tripStartDate.getTime()) / 86400000) + 1;
                        const daysPerSection = Math.max(
                            1,
                            Math.floor(totalDays / aiData.sections.length)
                        );
                        const startDay = index * daysPerSection;
                        const endDay = Math.min(startDay + daysPerSection - 1, totalDays - 1);
                    ...
                    const fullDesc = [section.description || "AI-generated content", ...extraBlocks]
                        .filter(Boolean)
                        .join("\n\n");
    ...
    const parseResponseManually = (text: string) => {
        ...
        lines.forEach(line => {
            ...
            if (trimmedLine.match(/^(Day \d+|Section \d+|Accommodation|Transportation|Activities|Dining)/i)) {
                if (currentSection.title) {
                    ...
                    const dateRange = sectionStartDate.toDateString() === sectionEndDate.toDateString()
                        ? format(sectionStartDate, "MMM d, yyyy")
                        : `${format(sectionStartDate, "MMM d")} - ${format(sectionEndDate, "MMM d, yyyy")}`;
        ...
        if (currentSection.title) {
            const sectionStartDate = new Date(currentDate);
            const sectionEndDate = tripEndDate ? new Date(tripEndDate) : new Date(currentDate);
    ...
    const addSection = () => {
        ...
        const newSection: Section = {
            ...
            description: "This section will be populated by AI recommendations.\nAI will suggest activities, accommodation, or travel details",
            dateRange: defaultDateRange,
            budget: "",
            startDate: tripStartDate,
            endDate: tripEndDate,
        ...
        setSections((prev) =>
            prev.map((section) => {
                ...
                if (
                    ...
                    const sd = field === "startDate" ? (value as Date | null) : section.startDate;
                    const ed = field === "endDate" ? (value as Date | null) : section.endDate;
                    ...
                    if (sd && ed) {
                        ...
                        // Format date range
                        if (updated.startDate!.toDateString() === updated.endDate!.toDateString()) {
                            updated.dateRange = format(updated.startDate!, "MMM d, yyyy");
                        } else {
                            updated.dateRange = `${format(updated.startDate!, "MMM d")} - ${format(updated.endDate!, "MMM d, yyyy")}`;
                        }
                    } else if (sd) {
    ...
    const sectionDateIssues = sections.some((s) => {
        if (!tripStartDate || !tripEndDate || !s.startDate || !s.endDate) return false; // only validate when full window exists
    ...
    useEffect(() => {
        ...
        if (duplicateTrip) {
            ...
            if (duplicateTrip.plannedBudget != null) setOverallBudget(String(duplicateTrip.plannedBudget));
            if (Array.isArray(duplicateTrip.sections) && duplicateTrip.sections.length) {
                setSections(
                            ...
                            dateRange: s.dateRange ||
                                (sd && ed
                                    ? (sd.toDateString() === ed.toDateString()
                                        ...
                                        : `${format(sd, "MMM d")} - ${format(ed, "MMM d, yyyy")}`)
                                    : defaultDateRange),
                            budget: s.budget != null ? String(s.budget) : (s.estimatedCost != null ? String(s.estimatedCost) : ""),
                            startDate: sd,
                            endDate: ed,
                        } as Section;
                    })
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => {
                                                if (!destination) {
                                                    ...
                                                    setTripTitle((prev) =>
                                                        prev.startsWith("AI Generated Trip") || prev.endsWith(" AI Trip")
                                                            ? (e.target.value ? e.target.value + " AI Trip" : "AI Generated Trip")
                                                            : prev
                                                    );
                                                }
                                            }}
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 sm:items-center sm:justify-between mt-12 py-4 border-t border-border/60">
                            ...
                            <div className="flex gap-3">
                                ...
                                <Button
                                    disabled={!canSave}
                                    onClick={() => {
                                        console.log(isEditing ? "🔄 Updating AI trip..." : "💾 Saving AI trip...");
                                        ...
                                        const fallbackStart = sectionDates.length
                                            ? new Date(Math.min(...sectionDates.map(d => d.getTime())))
                                            : new Date();
                                        const fallbackEnd = sectionEndDates.length
                                            ? new Date(Math.max(...sectionEndDates.map(d => d.getTime())))
                                            : fallbackStart;
                                        ...
                                        const effectiveDestinationFinal = (effectiveDestination && effectiveDestination.trim())
                                        ...
                                        try {
                                            const tripSections = sections.map((s) => ({
                                                ...
                                                startDate: s.startDate ? s.startDate.toISOString().substring(0,10) : undefined,
                                                endDate: s.endDate ? s.endDate.toISOString().substring(0,10) : undefined,
                                            }));

                                            if (isEditing && updateTrip && editTrip?.id) {
                                                // UPDATE EXISTING TRIP
                                                const updateData = {
                                                    title: tripTitle.trim() || effectiveDestinationFinal + " AI Trip",
                                            ...
                                            // Reset button state
                                            if (button) {
                                                button.textContent = isEditing ? "Update Trip" : "Save AI Itinerary";
                                                button.disabled = false;
                                                button.style.backgroundColor = "";
                                            }
                                            
                                            alert(`Failed to ${isEditing ? 'update' : 'save'} trip. Please try again.`);
                                        }
                                    }}
                                    data-save-button
